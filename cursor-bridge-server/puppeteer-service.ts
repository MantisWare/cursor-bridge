...
/**
 * Configuration interface for the Puppeteer service (the brain of browser automation! 🧠🤖)
 */
...
// Browser support notes (our browser family! 👨‍👩‍👧‍👦):
// - Chrome/Chromium: Fully supported (our favorite child! 🥇)
// - Edge: Fully supported (Chromium-based, so it's family! 👨‍👩‍👧‍👦)
// - Brave: Fully supported (Chromium-based, the brave one! 🦸‍♂️)
// - Firefox: Partially supported (the quirky cousin! 🦊)
// - Safari: Not supported by Puppeteer (the distant relative we don't talk about! 🦁)
...
/**
 * Configure the Puppeteer service with custom settings (make it your own! 🎨✨)
...
export function configurePuppeteerService(
  ...
  console.log("🎉 Puppeteer service configured with style:", currentConfig);
}

// ===== Browser Management =====

/**
 * Get or create a headless browser instance (the invisible browser that does all the work! 👻🌐)
 * @returns Promise resolving to a browser instance
 */
async function getHeadlessBrowserInstance(): Promise<puppeteer.Browser> {
  console.log("🚀 Browser instance request started - let's get this invisible browser party started!");
  ...
  if (headlessBrowserInstance) {
    try {
      ...
      console.log(
        `♻️ Reusing existing headless browser with ${pages.length} pages (efficiency is our middle name!)`
      );
      return headlessBrowserInstance;
    } catch (error) {
      console.log(
        "🔄 Existing browser instance is no longer valid, creating a new one (time for a fresh start!)"
      );
...
async function launchNewBrowser(): Promise<puppeteer.Browser> {
  console.log("🆕 Creating new headless browser instance - the invisible browser is being born!");
  ...
  try {
    ...
    console.log("🎉 Browser ready - the invisible browser is alive and ready to work!");
    return browser;
  } catch (error) {
    console.error("😅 Failed to launch browser:", error, "- even invisible browsers have their off days!");
