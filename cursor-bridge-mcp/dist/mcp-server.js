...
async function startServerSubprocess() {
    return new Promise((resolve) => {
        try {
            ...
            const workspaceRoot = path_1.default.join(__dirname, "..", "..");
            const workspacePackageJson = path_1.default.join(workspaceRoot, "package.json");
            const possibleServerPath = path_1.default.join(workspaceRoot, "cursor-bridge-server");
            const possibleServerExecutable = path_1.default.join(possibleServerPath, "dist", "browser-connector.js");
            // Only use local server if we're in the actual development workspace
            if (fs_1.default.existsSync(workspacePackageJson) &&
                fs_1.default.existsSync(possibleServerPath) &&
                fs_1.default.existsSync(possibleServerExecutable)) {
                try {
                    const packageJson = JSON.parse(fs_1.default.readFileSync(workspacePackageJson, "utf8"));
                    if (packageJson.name === "cursor-bridge" && packageJson.workspaces) {
                        console.log("Found local development workspace, using local server...");
                        serverProcess = (0, child_process_1.spawn)("node", [possibleServerExecutable], {
                    ...
                    serverProcess = (0, child_process_1.spawn)("npx", ["@mantisware/cursor-bridge-server"], {
                        stdio: ["ignore", "pipe", "pipe"],
                        detached: false,
                    });
                }
...
async function checkServerAtAddress(host, port) {
    try {
...
// We'll define our tools that retrieve data from the browser connector (the fun stuff! 🎉)
server.tool("getConsoleLogs", "Peek into your browser's console logs - see what's happening behind the scenes! 👀", async () => {
...
server.tool("getConsoleErrors", "Catch those sneaky console errors that are causing trouble! 🐛✨", async () => {
    return await withServerConnection(async () => {
...
server.tool("getNetworkErrors", "Hunt down those network errors that are making your app cry! 😭🌐", async () => {
    return await withServerConnection(async () => {
...
server.tool("getNetworkLogs", "Spy on all the network traffic - see what your app is really up to! 🕵️‍♀️📡", async () => {
    return await withServerConnection(async () => {
...
server.tool("takeScreenshot", "Snap a perfect screenshot of your browser tab - capture the moment! 📸✨", async () => {
    return await withServerConnection(async () => {
        try {
            ...
            if (response.ok) {
                return {
                    content: [
                        {
                            type: "text",
                            text: "🎉 Screenshot captured successfully! Your browser moment is now immortalized! 📸",
                        },
                    ],
                };
            }
                ...
                return {
                    content: [
                        {
                            type: "text",
                            text: `Oops! Screenshot failed: ${result.error} - even the best photographers have bad days! 😅`,
                        },
                    ],
                };
            }
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: `Screenshot mission failed: ${errorMessage} - the camera shy browser strikes again! 📷😅`,
                    },
                ],
            };
        }
...
server.tool("getSelectedElement", "Inspect the element that caught your eye - what's the user really clicking on? 👀🎯", async () => {
    return await withServerConnection(async () => {
...
server.tool("wipeLogs", "Clean slate time! Wipe all those browser logs and start fresh! 🧹✨", async () => {
    return await withServerConnection(async () => {
...
// Add tool for accessibility audits, launches a headless browser instance (making the web inclusive! ♿✨)
server.tool("runAccessibilityAudit", "Make your site accessible to everyone! Run a comprehensive accessibility audit that actually cares! ♿❤️", {}, async () => {
    return await withServerConnection(async () => {
        try {
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: `Accessibility audit hit a snag: ${errorMessage} - even the most caring audits need a little help sometimes! ♿💪`,
                    },
                ],
            };
        }
...
// Add tool for performance audits, launches a headless browser instance (speed demon mode! ⚡🚀)
server.tool("runPerformanceAudit", "Turn your site into a speed demon! Run a performance audit that hunts down those sneaky bottlenecks! ⚡🏃‍♂️", {}, async () => {
    return await withServerConnection(async () => {
        try {
            // Simplified approach - let the browser connector handle the current tab and URL
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: `Performance audit got tired: ${errorMessage} - even speed demons need a coffee break! ⚡☕`,
                    },
                ],
            };
        }
...
// Add tool for SEO audits, launches a headless browser instance (Google will love you! 🔍💕)
server.tool("runSEOAudit", "Make Google fall in love with your site! Run an SEO audit that turns you into a search engine whisperer! 🔍💕", {}, async () => {
    return await withServerConnection(async () => {
        try {
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: `SEO audit got stage fright: ${errorMessage} - even search engine whisperers have off days! 🔍😅`,
                    },
                ],
            };
        }
...
server.tool("runNextJSAudit", "NextJS superpowers activated! Run a specialized audit for your NextJS app! ⚛️🚀", {}, async () => ({
    content: [
        {
            type: "text",
            text: `
...
server.tool("runDebuggerMode", "Enter detective mode! Debug your app like a pro with our comprehensive debugging toolkit! 🕵️‍♀️🔍", async () => ({
    content: [
        {
            type: "text",
            text: `
...
server.tool("runAuditMode", "Enter audit mode! Run the full suite of audits to make your app absolutely amazing! 🎯✨", async () => ({
    content: [
        {
            type: "text",
            text: `
...
// Add tool for Best Practices audits, launches a headless browser instance (following the rules with style! ✅🎨)
server.tool("runBestPracticesAudit", "Check if you're following web development best practices - with love, not judgment! ✅💕", {}, async () => {
    return await withServerConnection(async () => {
        try {
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: `Best Practices audit needs a pep talk: ${errorMessage} - even the most well-behaved audits have their moments! ✅💪`,
                    },
                ],
            };
        }
